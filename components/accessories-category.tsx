...
const accessoriesCategories = categories.find((cat) => cat.name === "Accessories")
...
export default function AccessoriesCategory() {
    ...
    const getSubcategoryDescription = (subcategory: string) => {
        const descriptions: Record<string, string> = {
            "Women's Jwellery": "Elegant | Fashion-forward designs",
            "Men's Jwellery": "Bold | Sophisticated styles",
            "Men's Accessories": "Functional | Premium craftsmanship",
            "Women's Accessories": "Trendy | Statement pieces",
        }
    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-4 py-4 select-none">
            ...
            <div className="text-center mb-8">
                ...
                <StylizedText className="text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 bg-clip-text text-transparent mb-2">
                    {accessoriesCategories.name}
                </StylizedText>
            </div>
            ...
            <div className="relative">
                <div className="flex flex-row overflow-x-scroll scrollbar-hide gap-3 md:gap-6 pb-2">
                    {accessoriesCategories.subcategories?.map((subcategory, index) => {
                        const slug = createSlug(subcategory)
                        const imageUrl = getSubcategoryImage(subcategory, accessoriesCategories.name)
                                ...
                                <CardContent className="p-0 h-full">
                                    <div className="flex flex-col h-full">
                                        ...
                                        <div className="relative h-48 md:h-56 overflow-hidden">
                                            ...
                                            />
                                            {/* Gradient Overlay */}
                                            <div className="absolute inset-0 bg-gradient-to-t from-amber-400 via-yellow-200/20 to-transparent transition-opacity duration-300 group-hover:from-yellow-500 group-hover:via-amber-300"></div>

                                            {/* Category Title Overlay */}
                                            <div className="absolute top-4 left-4 right-4">
                                                <h3 className="text-xl md:text-2xl font-bold text-black drop-shadow-lg group-hover:text-amber-900 transition-colors duration-300">
                                                    {subcategory}
                                                </h3>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex-1 p-4 md:p-6 flex flex-col justify-between bg-white">
                                            <div className="mb-4">
                                                ...
                                                {accessoriesCategories.subsubcategories?.[subcategory] && (
                                                    <div>
                                                        <p className="text-xs text-gray-500 mb-2">Popular items:</p>
                                                        <div className="flex flex-wrap gap-1">
                                                            {accessoriesCategories.subsubcategories[subcategory].slice(0, 3).map((item, idx) => (
                                                                <span
                                                                    key={idx}
                                                                    className="text-xs bg-gradient-to-r from-yellow-100 to-amber-100 text-gray-700 px-2 py-1 rounded-full border border-yellow-200"
                                                                >
                                                                    {item}
                                                                </span>
                                                            ))}
                                                            {accessoriesCategories.subsubcategories[subcategory].length > 3 && (
                                                                <span className="text-xs text-gray-500 px-2 py-1">
                                                                    +{accessoriesCategories.subsubcategories[subcategory].length - 3} more
                                                                </span>
                                                            )}
                                                        </div>
                                                    </div>
                                            ...
                                            <Link href={`/subcategory/${slug}`} className="w-full">
                                                <Button className="w-full bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-600 hover:to-amber-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300 group-hover:shadow-lg transform group-hover:translate-y-[-1px]">
                                                    SHOP NOW
